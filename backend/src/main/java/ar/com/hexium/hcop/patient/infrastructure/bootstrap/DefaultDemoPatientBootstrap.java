...
public class DefaultDemoPatientBootstrap {
  ...
  private final PostgresPatientStore patients;
  private final PostgresPatientDocumentRepository documents;
  private final PatientDocumentStoreAdapter documentStamper;
  private final ObjectMapper mapper;
  private final boolean enabled;
  private final Resource clinicalDocumentResource;

  public DefaultDemoPatientBootstrap(
      PostgresPatientStore patients,
      PostgresPatientDocumentRepository documents,
      PatientDocumentStoreAdapter documentStamper,
      ObjectMapper mapper,
    ...
    this.documentStamper = documentStamper;
    this.mapper = mapper;
  ...
  private void prepareManagedDocument(
    ...
    documentStamper.applyPatient(document, patient, managedRevision);
  ...
  private Long bootstrapActorOrWarn(String action) {
    Long actorId = patients.findMinEnabledActorId().orElse(null);
    if (actorId == null) {
      LOGGER.warn(
