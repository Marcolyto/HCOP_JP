...
/**
 * Persistencia cruda de la historia clínica — sin ninguna regla de negocio (eso vive en
 * {@link PatientDocumentStoreAdapter}, el mismo split que tenían
 * {@code PatientDocumentRepository}/{@code PatientDocumentService} antes de F3.3, preservado
 * a propósito así los contract-tests que mockean solo la persistencia y ejercitan la lógica
 * real siguen funcionando igual).
 */
@Repository
public class PostgresPatientDocumentRepository {
  private final JdbcTemplate jdbc;
  private final ObjectMapper mapper;
  private final Clock clock;

  public PostgresPatientDocumentRepository(JdbcTemplate jdbc, ObjectMapper mapper, Clock clock) {
    this.jdbc = jdbc;
    this.mapper = mapper;
    this.clock = clock;
  }
  ...
  public Optional<StoredDocument> insertIfMissing(
      long patientId, JsonNode document, long actorId, boolean imported) {
    Instant now = clock.instant();
    return jdbc.query("""
        INSERT INTO hcop_patient_documents
          (patient_id, document_json, revision, imported_at, created_by, updated_by, created_at, updated_at)
  ...
  public Optional<StoredDocument> update(
      long patientId, JsonNode document, long expectedRevision, long actorId) {
    Instant now = clock.instant();
    return jdbc.query("""
  ...
  /** JsonNode real (no {@code Object} opaco) — colaborador interno entre dos clases de
   * infraestructura, {@link PatientDocumentStoreAdapter} lo convierte al tipo de dominio. */
  public record StoredDocument(
      long patientId, JsonNode document, long revision, Instant importedAt, Instant createdAt,
      Instant updatedAt) {
  }
}
