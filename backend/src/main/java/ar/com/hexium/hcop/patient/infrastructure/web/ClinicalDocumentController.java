...
public class ClinicalDocumentController {
  ...
  ResponseEntity<JsonNode> get(HttpServletRequest request) {
    ...
    JsonNode state = (JsonNode) (principal.activePatientId() == null
  ...
  ResponseEntity<Map<String, Object>> put(
    ...
    JsonNode currentDocument = (JsonNode) current.document();
    JsonNode stateToSave = accessPolicy.writableState(state, currentDocument, principal);
    ...
    changeValidator.validate(stateToSave, currentDocument);
    stateToSave = summaryPlanAuthority.canonicalize(stateToSave, currentDocument, principal);
    ...
    StoredDocument saved = documents.save(
        ...
        expected,
        principal.userId());
    JsonNode canonicalState = accessPolicy.visibleState((JsonNode) documents.state(saved), principal);
    return ResponseEntity.ok()
        .cacheControl(CacheControl.noStore())
        .body(Map.of(
            "ok", true,
