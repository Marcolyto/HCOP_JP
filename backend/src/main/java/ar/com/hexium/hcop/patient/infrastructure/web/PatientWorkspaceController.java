...
public class PatientWorkspaceController {
  ...
  private Map<String, Object> workspace(long patientId, SessionPrincipal principal) {
    ...
    JsonNode state = accessPolicy.visibleState((JsonNode) documents.state(stored), principal);
    ...
    response.put("patient", json.patientView(patient));
    response.put("state", state);
    response.put("document", document);
    response.put("revision", stored.revision());
    response.put("updatedAt", stored.updatedAt().toString());
    response.put("counts", json.counts(state));
    response.put("completeness", json.completeness());
