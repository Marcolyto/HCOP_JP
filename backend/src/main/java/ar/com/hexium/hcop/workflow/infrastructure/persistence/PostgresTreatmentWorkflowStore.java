...
@Repository
public class PostgresTreatmentWorkflowStore implements TreatmentWorkflowStore {
  private final JdbcTemplate jdbc;
  private final ObjectMapper mapper;

  public PostgresTreatmentWorkflowStore(JdbcTemplate jdbc, ObjectMapper mapper) {
    this.jdbc = jdbc;
    this.mapper = mapper;
  }
  ...
  @Override
  public Optional<TreatmentWorkflowSummary> treatment(long patientId, String treatmentId) {
    return jdbc.query("""
        ...
        """, this::mapTreatment, patientId, treatmentId).stream().findFirst();
  }
  ...
  public long insertRequest(
    ...
    context.forEach(contextNode::put);
    try {
      return jdbc.queryForObject("""
          ...
          message, contextNode.toString());
    } catch (DataIntegrityViolationException duplicate) {
      throw new TreatmentWorkflowStore.DuplicateRequestException(
          "Ya existe una solicitud pendiente para este tratamiento y ciclo.");
    }
  }

  @Override
  public Optional<WorkflowRequest> request(long id) {
    return jdbc.query(requestSelect() + " WHERE w.id = ?", this::mapRequest, id).stream().findFirst();
  }

  @Override
  public List<WorkflowRequest> inbox(long userId) {
  ...
  @Override
  public Optional<WorkflowRequest> markSeen(long id, long userId, Instant now) {
  ...
  @Override
  public Optional<WorkflowRequest> resolve(
  ...
  public void insertEvent(
    ...
    jdbc.update("""
        ...
        """, UUID.randomUUID(), requestId, patientId, treatmentId, cycle, eventType, actorId,
        ((JsonNode) event).toString());
  }
  ...
  private TreatmentWorkflowSummary mapTreatment(ResultSet result, int row) throws SQLException {
  ...
  private WorkflowRequest mapRequest(ResultSet result, int row) throws SQLException {
    Date resume = result.getDate("resume_date");
